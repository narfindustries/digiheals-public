...
class TestHapiClient:
    ...
    @pytest.mark.parametrize("step_number, filename", [
        (0, "./test_files/Abbie917_Frami345_ffa07c38-1f19-6336-9952-9152a6c882c9.json"),
    ...
    def test_step(self, hapi_client, step_number, filename):
        ...
        if step_number == 0:
            with open(filename, 'r') as file:
                patient_id, response_json, export_response = hapi_client.step(step_number, file)
        else:
            with open(filename, 'r') as file:
                data = json.load(file)
                patient_id, response_json, export_response = hapi_client.step(step_number, data)
