...
@pytest.mark.parametrize("guid, depth", 
    [
    ('68889840-0eb3-4e95-b1a7-0d3721714915', 1), # JSON depth 1 3d2f0488-7588-414c-be04-664b394366ff (vista)
...
@pytest.mark.parametrize("file1, file2, file_type, expected", 
    [
    ('./test_files/json_diff_file1_id.json', './test_files/json_diff_file2_id.json', 'json', (True, "./test_files/json_diff_1_2_id.txt")),  
    ('./test_files/xml_diff_file1.txt', './test_files/xml_diff_file2.txt', 'xml', (False, "./test_files/xml_diff_1_2.txt")),
    ('./test_files/json_diff_file1.json', './test_files/json_diff_file2.json', 'json', (False, "./test_files/json_diff_1_2.txt")),   
    ])
...
def test_compare_paths_with_chains(capsys):
    ...
    assert "45038f5e-db56-49ac-9f72-6d212917ac6b" in captured.out, "GUID not found in output"
    assert "file -> hapi and hapi -> blaze" in captured.out, "Chains not found in output"
...
def test_cli_options():
    ...
    runner = CliRunner()
    test_args = ["--guid", "3d2f0488-7588-414c-be04-664b394366ff", "--depth", "1", "--type", "json"]
    
    result = runner.invoke(diff_cli_options, test_args) # Invoke CLI command
    
    assert result.exit_code == 0, "CLI exited with non-zero status" # CLI option evoked correctly but returns SystemExit=0
...
def test_database_integration():
    ...
    assert result is None, "Full integration failed." # Return is None, as this function doesn't return anything
