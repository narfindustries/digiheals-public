...
def cli_options(check_connections, chain_length, file, generate, chain, all_chains, fuzz):
    ...
    if file:
        fileobj = file
        file = file.read()
    synthea_url = "http://localhost:9000"
    if generate:
        first_node = "synthea"  # generated by Synthea, not a file read
        r = requests.get(synthea_url, timeout=100)
        if r.status_code == 200:
            filename = r.json()["filename"]
            # overwrite the file variable
            fileobj = open(f"../files/fhir/{filename}")
            file = fileobj.read()
            ...
            sys.exit(1)
    if fuzz:
        fuzz_client = FuzzClient(synthea_url)
        filename = Path(fileobj.name).name
        res, data = fuzz_client.fuzz(filename)
        rlts = data.get('results', [])
        if res != 200 or not rlts:
            print("Failed to fuzz file %s. Can only fuzz generated files or files in files/fhir subdirectory",
                  fileobj.name)
            sys.exit(1)
        # wait for fuzz to finish before reading file
        fuzz_client.wait_until_done(filename)

        # read in fuzzed file contents
        with open(f"../files/fuzzed/{rlts[0]['output_name']}", "r") as f:
            print("Playing telephone with file: %s", f.name)
            file = f.read()
