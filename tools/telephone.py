...
def dfs(guid, first_node, counter, step, chain, chain_length, file, file_type):
    ...
    if len(chain) > 0:
        ...
        (error, file) = process_step(
            guid, first_node, counter - 1, step, tmp_chain, file, chain_length, file_type
        )
    ...
    for node in list(config.keys()):
        dfs(guid, first_node, counter + 1, node, chain + [node], chain_length, file, file_type)


def process_step(guid, first_node, step_number, step, chain, file, chain_length, file_type):
    """
