    ...
    for key, details in metadata.items():
        if "type" in details and details["type"][0].isupper():  # Check if type is a nested type
            ...
            if key in data_to_parse:  # Check if this field exists in the patient data
                ...
                nested_spec_file = (
                    resource_nested_spec_file if os.path.exists(resource_nested_spec_file) else type_nested_spec_file
                )
                ...
                if os.path.exists(nested_spec_file):
                    ...
                    if isinstance(value, list):  # If its a list, use for loop to traverse through each element
                        for list_element in value:
                            if isinstance(list_element, dict):
                                temp_depth, temp_chain = max_depth_traverse(list_element, nested_depth, nested_spec_file, graph, nested_chain)
                                if temp_depth > max_depth:
                                    max_depth = temp_depth
                                    longest_chain = temp_chain

                    elif isinstance(value, dict):
                        temp_depth, temp_chain = max_depth_traverse(value, nested_depth, nested_spec_file, graph, nested_chain)
...
for synthea_file in synthea_patient_files:
    ...
    current_node = os.path.basename(synthea_file).split("_")[0] + '_' + os.path.basename(synthea_file).split("_")[1]
    ...
    for entry in data.get("entry", []):
        ...
        if os.path.exists(resource_spec_file):
            ...
            max_depth, longest_chain = max_depth_traverse(resource, 1, resource_spec_file, graph, nested_chain_path)
    ...
    patient_data[file_name] = {"num_nodes":len(graph.nodes()), 
