...
class OutputTree:
    ...
    def _test_output_path(self, path, ehr_name):
        path = Path(path)
        output_dir = self.output_dir / path
        return output_dir / Path(f"{ehr_name}.json")

    def test_ehr_path(self, path, ehr_name):
        return self._test_output_path(path, ehr_name)
    ...
    def run(self, diff_ignore_order=False, parsers=None):
        ...
        for item in self.tree.iter_results_dir():
            ...
            for result in item.iterdir():
                if (
                    parsers
                    and result.stem not in parsers
            ...
            print(
                json.dumps(results, sort_keys=True, indent=1, cls=ResultEncoder),
                "}",
                flush=True,
            )
        print("]")
...
class TestManager:
    ...
    def test_file(self, path, force=False):
        ...
        for ehr in EHRMapping.iter_unique():
            output_file = self.tree.test_ehr_path(dst, ehr.ehr)
...
@click.option(
    "-i",
    "--input",
    type=click.Path(dir_okay=False, path_type=Path, readable=True),
    default="output.json",
    help="Path to json file generated by `analyze` command",
)
def filter(input):
...
@click.option(
    "-o",
    "--output-dir",
    type=click.Path(file_okay=False, path_type=Path, writable=True),
    default="output",
...
@click.option(
    "-p",
    "--parser",
    type=click.Choice([v.value for v in Parsers]),
    multiple=True,
    help="Parers to analyze, default is all",
)
@click.option(
    "--ignore-order", is_flag=True, help="Ignore order when performing deep diff"
)
...
@click.option(
    "-i",
    "--input-dir",
    type=click.Path(file_okay=False, path_type=Path, readable=True),
    help="directory containing json files to be tested, will recursivel walk",
    required=True,
)
@click.option(
    "-o",
...
@click.option(
    ...
    help="Base url of python parser",
)
@click.option(
    "-c",
    "--clojureurl",
    default="http://localhost:8282",
    help="Base url of clojure parser",
)
@click.option(
    "-v", "--vistaurl", default="http://localhost:8383", help="Base url of vista parser"
)
@click.option(
    "-j", "--javaurl", default="http://localhost:8181", help="Base url of java parsers"
)
@click.option(
    "-f",
    "--force",
    is_flag=True,
    help="force recreation of result file even if it exists",
)
