...
class FuzzInputFilePath(click.Path):
    ...
    name = "file"
    def __init__(self):
        super().__init__(dir_okay=False, exists=True, resolve_path=True,
                         path_type=Path)
        
    def convert(self, value, param, ctx):
        value = super().convert(value, param, ctx)
        if value.parent == None or value.parent.parent == None or \
           value.parent.name != "fhir" or value.parent.parent.name != "files":
            self.fail(f"{value} must have parent directory of 'files/fhir/'", param, ctx)
        return value.name
...
def fuzz(client, filename, count, no_wait):
    ...
    if res != 200 or not rlts:
        print(f"Failed to fuzz file {filename}. ",
...
@optgroup.option("--file", type=FuzzInputFilePath(),
...
@optgroup.option("--is-fuzzing", is_flag=True, default=False,
                 help="Check if fuzzing jobs are still pending")
@click.option("--url", type=str, default="http://localhost:9000",
