...
import docker
import time
from utils.meta_tag import inject_meta_tag
...
def telephone_function(
    ...
    if code_tag == "default_tag" and ("ibm" in chain or "iris" in chain):
        # Need to inject meta tag for IBM and Iris
        inject_meta_tag(file.name, guid)
        print("Successfully injected meta tag into the file")
        code_tag = guid
    ...
    if generate:
        ...
        process_chain(guid, first_node, chain, file, file_type, code_tag)
