...
class IBMFHIRClient(AbstractClient):
    ...
    def export_patients(self, file_type=None):
        ...
        try:
            # Adding _count=500 here. Otherwise, it only returns a max of 10 bundle resources,
            # even though more bundles are being created. Max returned in one page is 10000.
            r = requests.get(
    ...
    def export_patients_tag(self, tag=None, file_type=None):
        """Calls the FHIR API to export a patient record that contains a specific tag"""
        if file_type is None:
            # Used for checking network/default
            file_type = "json"
        header_text = "application/fhir+" + file_type
