...
for filename in os.listdir(folder_path):
    if filename.endswith(".json"):
        ...
        curl_command_ibm = [
            ...
            "-X", "POST", "https://localhost:9443/fhir-server/api/v4/Bundle",
        ...
        curl_command_iris = [
            "curl", "-u", "_system:SYS",
            "-X", "POST", "http://localhost:8007/fhir/r4/Bundle",
            "-H", "Content-Type: application/fhir+json",
            "-d", f"@{file_path}"
        ]
        ...
        curl_command_vista = [
                    "curl",
                    "-X", "POST", "http://localhost:9080/addpatient",
                    "-H", "Content-Type: application/fhir+json",
                    "-d", f"@{file_path}"
                ]
        ...
        curl_command_hapi = [
                    "curl",
                    "-X", "POST", "http://localhost:8004/fhir/Bundle",
                    "-H", "Content-Type: application/fhir+json",
                    "-d", f"@{file_path}"
                ]
